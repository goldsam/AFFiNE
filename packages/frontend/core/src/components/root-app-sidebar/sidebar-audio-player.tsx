...
export const SidebarAudioPlayer = () => {
  ...
  const audioAttachmentRendering = useLiveData(
    useMemo(() => {
      if (!audioAttachmentBlockEntity) {
        return null;
      }
      return LiveData.from(
        audioAttachmentBlockEntity.rendering$.pipe(
          map(v => isSameTab && v),
          debounceTime(50)
        ),
        true
      );
    }, [audioAttachmentBlockEntity, isSameTab])
  );
