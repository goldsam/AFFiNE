  ...
  actions: [
    ...
    },
    {
      ...
      generate(ctx) {
        ...
        const shouldShowTooltip$ = computed(
          () => firstModel.displayMode$.value === NoteDisplayMode.DocAndEdgeless
        );
        const label$ = computed(() =>
          firstModel.displayMode$.value === NoteDisplayMode.EdgelessOnly
            ? 'Display In Page'
            : 'Displayed In Page'
        );
        const onSelect = () => {
          const newMode =
            firstModel.displayMode === NoteDisplayMode.EdgelessOnly
    ...
    },
    {
      ...
      when(ctx) {
        ...
        return (
          elements.length > 0 &&
          elements[0].displayMode !== NoteDisplayMode.DocOnly
        );
      },
      content(ctx) {
        const models = ctx.getSurfaceModelsByType(NoteBlockModel);
        ...
        return html`
          <edgeless-color-picker-button
            ...
            .originalColor=${firstModel.background}
      ...
      actions: [
        {
          ...
          content(ctx) {
            ...
            const theme = ctx.themeProvider.edgelessTheme;
            const firstModel = models[0];
            const { shadowType } = firstModel.edgeless.style;
            const background =
              getMostCommonValue(
                models.map(model => ({
                  background: resolveColor(model.background, theme),
        ...
        } satisfies ToolbarAction,
        {
          ...
          content(ctx) {
            ...
            const firstModel = models[0];
            const { borderSize, borderStyle } = firstModel.edgeless.style;
        ...
        } satisfies ToolbarAction,
        {
          ...
          content(ctx) {
            ...
            const borderRadius$ = computed(
              () => firstModel.edgeless$.value.style.borderRadius
            );
    ...
    },
    {
      ...
      generate(ctx) {
        ...
        const firstModel = models[0];
        const { collapse } = firstModel.edgeless$.value;
        ...
        return {
          ...
          run(ctx) {
            for (const model of models) {
              const edgeless = model.edgeless;

              if (edgeless.collapse) {
                ctx.store.updateBlock(model, () => {
                  model.edgeless.collapse = false;
                });
                continue;
              }
    ...
    },
    {
      ...
      content(ctx) {
        ...
        const scale$ = computed(() => {
          const scale = firstModel.edgeless$.value.scale ?? 1;
        ...
        const onSelect = (e: CustomEvent<number>) => {
          ...
          models.forEach(model => {
            const bounds = Bound.deserialize(model.xywh);
            const oldScale = model.edgeless.scale ?? 1;
